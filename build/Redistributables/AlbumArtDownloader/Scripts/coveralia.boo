...
class Coveralia:
	...
	static def GetThumbs(coverart,artist,album):
		...
		resultResults = GetPageIsoLatin1(String.Format("https://www.coveralia.com/mostrar.php?bus={0}&bust=2", EncodeUrlIsoLatin1(query)))
		...
		for resultMatch as Match in resultMatches:
			//Get the result page
			resultPage = GetPageIsoLatin1(String.Format("https://www.coveralia.com{0}", resultMatch.Groups["url"].Value))
			
			labelRegex = Regex("<title>(?<title>[^<]+)", RegexOptions.IgnoreCase)
			labelMatch = labelRegex.Match(resultPage) //Expecting one match
			
			imagePageRegex = Regex("<a href=\"/caratulas/(?<imageName>[^\"]+)\"[^>]*><img src=\"https://images.coveralia.com/audio/thumbs/(?<thumbID>[^\"]+)\"")
			...
			for imagePageMatch as Match in imagePageMatches:
				...
				fullSizeImagePageUrl =String.Format("https://www.coveralia.com/caratulas/{0}", imagePageMatch.Groups["imageName"].Value)
				...
				fullSizeImageRegex = Regex("src=\"(?<url>https://images\\.coveralia\\.com/audio/[^\"]+)\"[^>]*class=\"caratula\"")
				fullSizeImageMatch = fullSizeImageRegex.Match(fullSizeImagePage) //Expecting only one match
				
				if fullSizeImageMatch.Success:
					coverart.Add(
						String.Format("https://images.coveralia.com/audio/thumbs/{0}", imagePageMatch.Groups["thumbID"].Value),
